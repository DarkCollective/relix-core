...
/**
 * Full outer join (⟗) — returns all tuples from both {@code left} and
 * {@code right}; unmatched tuples on either side are padded with nulls.
 *
 ...
 */
public record FullOuterJoinNode(RelNode left, RelNode right, Predicate condition, SourceLocation location) implements ConditionalJoinNode {
    ...
    public FullOuterJoinNode(RelNode left, RelNode right, Predicate condition) {
        this(left, right, condition, SourceLocation.UNKNOWN);
    }

    @Override
    public FullOuterJoinNode rebuild(RelNode left, RelNode right, Predicate condition) {
        return new FullOuterJoinNode(left, right, condition, location());
    }
