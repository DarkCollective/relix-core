...
/**
 * Logical conjunction (∧) — true when both {@code left} and {@code right} are
 * true.
 *
 * <p>Example: {@code (age > 18) ∧ (status = "active")}
 *
 * @param left     the left predicate; must not be null
 * @param right    the right predicate; must not be null
 * @param location the source location of this predicate; never null
 */
public record AndPredicate(Predicate left, Predicate right, SourceLocation location) implements Predicate {
    public AndPredicate {
