...
/**
 * Distinct (δ) — eliminates duplicate tuples from {@code input}.
 *
 * <p>Example: {@code δ (Orders)}
 *
 * @param input    the source relation; must not be null
 * @param location the source location of this node; never null
 */
public record DistinctNode(RelNode input, SourceLocation location) implements RelNode {
    public DistinctNode {
    ...
    public DistinctNode(RelNode input) {
