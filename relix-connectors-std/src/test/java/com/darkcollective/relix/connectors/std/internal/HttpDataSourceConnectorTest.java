...
import com.darkcollective.relix.connectors.std.internal.HttpDataSourceConnector.HttpFetchResult;
import com.darkcollective.relix.connectors.std.internal.HttpDataSourceConnector.HttpRequestSpec;
import com.darkcollective.relix.connectors.std.internal.HttpDataSourceConnector.HttpTransport;
...
final class HttpDataSourceConnectorTest extends ProcessorTestSupport {
    ...
    private static final class RecordingTransport implements HttpTransport {
        ...
        /** A transport whose client refused the request before sending it — a bad header, a bad URL. */
        static RecordingTransport refusing(IllegalArgumentException e) {
            return new RecordingTransport(e);
        }
    ...
    class RequestBuilding {
        ...
        @Test
        @DisplayName("QUERY sends its body with the QUERY method")
        void queryMethod() {
            var t = new RecordingTransport(200, "[]");
            exec("""
                    ...
                    query Search;
                    """, t);
            assertThat(t.lastRequest.method()).isEqualTo(HttpMethod.QUERY);
            assertThat(t.lastRequest.body()).contains("{\"status\": \"OPEN\"}");
        }
    }
    ...
    class ContentType {
        ...
        @Test
        @DisplayName("a request with a body is sent as application/json by default")
        void defaultedWhenThereIsABody() {
            var t = new RecordingTransport(200, "[]");
            exec("""
                    source Api from http { url: "https://x", method: POST, body: "{}" };
                    query Api;
                    """, t);
            assertThat(t.lastRequest.headers()).containsEntry("Content-Type", "application/json");
        }

        @Test
        @DisplayName("a declared Content-Type wins, whatever its spelling")
        void declaredValueWins() {
            var t = new RecordingTransport(200, "[]");
            exec("""
                    source Api from http {
                        url: "https://x", method: QUERY, body: "SELECT 1",
                        headers: { "content-type": "application/sql" }
                    };
                    query Api;
                    """, t);
            assertThat(t.lastRequest.headers())
                    .containsEntry("content-type", "application/sql")
                    .doesNotContainKey("Content-Type");
        }

        @Test
        @DisplayName("a GET with no body carries no Content-Type")
        void absentWithoutABody() {
            var t = new RecordingTransport(200, "[]");
            exec("source Api from http { url: \"https://x\" };\nquery Api;", t);
            assertThat(t.lastRequest.headers()).doesNotContainKey("Content-Type");
        }

        @Test
        @DisplayName("the generated GraphQL document is sent as application/json")
        void generatedGraphQl() {
            var t = new RecordingTransport(200, "{\"data\": {\"users\": []}}");
            exec("""
                    ...
                    query Users;
                    """, t);
            assertThat(t.lastRequest.body()).isPresent();
            assertThat(t.lastRequest.headers()).containsEntry("Content-Type", "application/json");
        }
    }
    ...
    class Errors {
        ...
        @Test
        @DisplayName("a request the client refuses is an evaluation error naming the relation")
        void refusedRequest() {
            var t = RecordingTransport.refusing(
                    new IllegalArgumentException("header 'X-Token': invalid header value"));
            assertThatThrownBy(() -> exec(
