 ...
 * @param maxProcessedRows   maximum rows one execution's operators may pass to one
 *                           another in total before aborting with an
 *                           {@link EvaluationException}: a deterministic measure of work,
 *                           which stops a query that runs long while producing little.
 *                           Use {@link #UNLIMITED_PROCESSED_ROWS} for no cap (the
 *                           default); must be &ge; 1
 * @param timeout            how long one execution may run before aborting with an
 *                           {@link EvaluationException}, checked as rows pass between
 *                           operators; use {@link #UNLIMITED_TIMEOUT} for none (the
...
public record ExecutionContext(
    ...
    public static final long UNLIMITED_PROCESSED_ROWS = Long.MAX_VALUE;

    /** No timeout on one execution, which is the default. */
    public static final Duration UNLIMITED_TIMEOUT = Duration.ofSeconds(Long.MAX_VALUE);
    ...
    public ExecutionContext {
        ...
        if (maxProcessedRows < 1) {
            throw new IllegalArgumentException(
                    "maxProcessedRows must be >= 1, was: " + maxProcessedRows);
        }
    ...
    /**
     * Returns a copy of this context with the given cap on the rows one execution may
     * process.  All other fields are shared unchanged.
     *
     * <p>A row is counted each time an operator hands one to the operator above it, so
     * the count is the sum of every operator's output. That is what stops the two queries
     * a cap on output cannot: a selection over an endless generator that matches
     * nothing, and an aggregate over a large product that emits a single row. It is a
     * running total for the whole execution, recursion included, because what it bounds
     * is work done rather than memory held.
     *
     * @param maxProcessedRows the maximum rows processed; must be &ge; 1; use
     *                         {@link #UNLIMITED_PROCESSED_ROWS} for no cap
     * @return a new context with the given limit
     */
    ...
    /**
     * Returns a copy of this context with the given limit on how long one execution may
     * run.  All other fields are shared unchanged.
     *
     * <p>The clock starts when execution does, not when planning does. It is checked as
     * rows pass between operators, so it cannot interrupt a call that does not return to
     * the engine: a JDBC driver waiting on its database, or a solver's search.
     *
     * @param timeout the longest one execution may run; must be positive; use
     *                {@link #UNLIMITED_TIMEOUT} for none
     * @return a new context with the given limit
     */
